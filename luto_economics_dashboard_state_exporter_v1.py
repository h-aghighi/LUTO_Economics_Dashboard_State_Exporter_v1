...
Purpose
-------
Create ONLY dashboard-style economics Excel tables, grouped as:
    ...
    output_dir/
      ...
      New_South_Wales/
        Run_G0001_New_South_Wales_Economics_Dashboard_Final_Table.xlsx
          Sheet: Central Tablelands
          ...

Each sheet uses the dashboard chart-export layout:

    Row 1: merged title, e.g. Economics overview for Fitzroy
    Row 2: Category | Agricultural Land-use (revenue) | ... | Profit
    Row 3+: years and values

This script intentionally does NOT write diagnostic CSVs, long tables, detail workbooks,
or ranking files. It only writes the final state workbooks.
...
Recommended input
-----------------
Use DATA_REPORT/data, not DATA_REPORT/data/map_layers.
If map_layers is supplied, the script automatically switches to the parent data folder.
"""
...
def find_window_assignment(js_text: str) -> Tuple[Optional[str], Optional[str]]:
    pattern = re.compile(r"window(?:\[['\"](?P<qname>[^'\"]+)['\"]\]|\.(?P<dname>[A-Za-z_$][\w$]*))\s*=\s*", re.MULTILINE)
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Write final LUTO economics dashboard tables grouped by state folder and region sheet.")
    parser.add_argument("--data-dir", required=True, help="Path to DATA_REPORT/data. If map_layers is supplied, parent data folder is used automatically.")
    parser.add_argument("--output-dir", required=True, help="Output folder.")
    parser.add_argument("--output-prefix", default="Run_G0001", help="Prefix used in workbook names, e.g. Run_G0001")
    ...
    parser.add_argument("--chart-title-template", default="Economics overview for {region}", help="Sheet title. Available fields: {region}, {state}")
    ...
    prefix = sanitize_filename(args.output_prefix)

    if not data_dir.exists():
        log(f"ERROR: data folder does not exist: {data_dir}")
        return 2

    econ_file = data_dir / DASHBOARD_FILE
    if not econ_file.exists():
        # Support recursive fallback, but prefer the direct data folder file.
        matches = list(data_dir.rglob(DASHBOARD_FILE))
        if not matches:
            log(f"ERROR: Could not find {DASHBOARD_FILE} under: {data_dir}")
            return 3
        econ_file = matches[0]

    var, parsed, err = parse_js(econ_file)
    if parsed is None or not isinstance(parsed, dict):
        log(f"ERROR: Could not parse {econ_file}")
        if err:
            log(err)
        return 4
    ...
    for region in sorted(parsed.keys()):
        if region == "AUSTRALIA" and not args.include_national:
            continue
        ...
        chart_df = make_region_chart_table(region_df, args.start_year, args.end_year)
        if chart_df.empty:
            continue
        by_state.setdefault(state, []).append((region, chart_df))

    if not by_state:
        log("ERROR: No region tables were created after filters.")
        return 5
    ...
    for state, region_tables in sorted(by_state.items()):
        ...
        for region, chart_df in region_tables:
            ...
            title = args.chart_title_template.format(region=region, state=state)
            write_chart_table_sheet(ws, chart_df, chart_title=title)
        wb.save(workbook_path)
        log(f"Saved: {workbook_path} ({len(region_tables)} region sheet(s))")

    if unknown_regions:
        log("WARNING: These regions were not in the built-in state mapping and were written to Unknown_State:")
